...
class FinanceiroClienteExport extends BaseExport implements FromView
{
    ...
    public function view(): View
    {
        ...
        return view('exports.financeiro-cliente', [
            'laudos' => Self::$LAUDOS,
            'sumario' => $sumario,
    ...
    private function sumario($laudos) {
        ...
        foreach ($laudos as $laudo) {
            $total[$laudo->tipo_exame] = isset($total[$laudo->tipo_exame])
                ? ( $total[$laudo->tipo_exame] + number_format($laudo->preco, 2) )
                : number_format($laudo->preco, 2);
        }
        $object = [];

...
HISTORY
[Version : 1.3.0]	[Date : 2016/10/27]     [Author : CV]
...
abstract class  PdfObjectBase		// extends  Object
{
    ...
    // parser)
    protected static  $FontSpecifiers	=  '(/F \d+) | (/R \d+) | (/f-\d+-\d+) | (/[CT]\d+_\d+) | (/TT \d+) | (/OPBaseFont \d+) | (/OPSUFont \d+)' ;
...
class  PdfToText extends PdfObjectBase
{
    ...
    public function  Load ( $filename, $user_password = false, $owner_password = false )
    {
        ...
        $memory_peak_usage_start	=  memory_get_peak_usage ( true ) ;
        ...
        $memory_peak_usage_end		=  memory_get_peak_usage ( true ) ;
    ...
    public function  ProcessTemplateReferences ( $page_number, $text_data )
    {
        ...
        if  ( isset ( $this -> Pages [ $page_number ] ) )
        {
            ...
            foreach  ( $this -> PageContents  as  $page_contents )
            {
                ...
                if  ( $page_contents [ 'page' ]  ==  $page_number  &&  count ( $page_contents [ 'xobjects' ] ) )
                {
                    $template_searches	=  [] ;
                    ...
                    {
                        if  ( isset ( $this -> TemplateObjects [ $template_object ] ) )
                        {
                            $template_searches []		=  '#' . $template_name . ' \s+ Do\b #msx' ;
                            $template_replacements []	=  $this -> TemplateObjects [ $template_object ] ;
                        }
                    }

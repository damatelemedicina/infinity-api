...
class RecadoController extends Controller
{
    ...
    public function mostraRecados(Request $request)
    {
        ...
        if ($isCliente) {
            $cliente = Cliente::selectRaw("emergencia, json")->where('id', $usuario->conta_cliente)->first();
            $ClienteJson = $cliente->json instanceof ClienteJson ? $cliente->json : new ClienteJson(null, $cliente->json ?? '[]');
            if ($cliente->emergencia == 1 && empty($ClienteJson->mensagem_urgencia_emergencia) == false) {
                $recados[] = ['recado' => $ClienteJson->mensagem_urgencia_emergencia, 'origem' => 'EMERGÊNCIA', 'color' => 'red'];
            }
        }
        $this->getRecadosEmpresa($empresa, $isColaborador, $isMedico, $isCliente, $recados);
        $this->getRecadosMatriz($empresa, $isColaborador, $isMedico, $isCliente, $recados);
        return $recados;
    }
